<template>
  <div>
    <div class="hint">
      灾情信息查询：<span v-if="type !== ''">{{ type }}</span><span v-else></span><i
        style="margin-left: 8px; font-size: 20px"
        :class="{
          'iconfont icon-bianhuajiance': type === '自然灾害变化检测',
    ...
    >
      ...
      <el-table-column label="监测区域">
        <template #default="scope">
          <span id="sub-title"><i
            :class="{
              'iconfont icon-bianhuajiance': scope.row.type === '自然灾害变化检测',
      ...
      <el-table-column label="整体区域损伤程度">
        <template #default="scope">
          <img
      ...
      <el-table-column label="局部区域损伤程度">
        <template #default="scope">
          <el-button
            size="default"
            type="danger"
