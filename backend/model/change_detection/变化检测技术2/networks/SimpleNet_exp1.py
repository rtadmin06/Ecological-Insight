...
from networks.modules.SimpleConv import SimpleConv
...
class DoubleConv(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.Conv = nn.Sequential(
            SimpleConv(in_ch, out_ch, dilation=3),
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True),
            SimpleConv(out_ch, out_ch, dilation=3),
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True)
        )
...
class SimpleNet_exp1(nn.Module):
    def __init__(self, in_ch, out_ch, ratio=0.5):
        super(SimpleNet_exp1, self).__init__()
