...
from networks.SimpleNet_exp3 import SimpleNet_exp3

TITLE = 'SimpleNet-exp3——CDD-1'
...
def main(args):
    net = SimpleNet_exp3(in_ch=3, out_ch=1, ratio=0.5).to(device)
    ...
    # net.load_state_dict(torch.load("/root/zyn/workspace/SimpleNet2_LEVIR_CD/ckps/SimpleNet-exp2——LEVIR_CD/SimpleNet-exp2——LEVIR_CD_batch=32_lr=0.0001_epoch430model.pth" , map_location='cuda:0'))
    start_epoch = 0
    total_epochs = 1000
    best_f1 = 0
    best_epoch = 0
    ...
    cosine_schedule = optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer,T_max=30,eta_min=0.00005)
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        print(f'Step {epoch}: Learning rate = {current_lr}')
        logger.info(
