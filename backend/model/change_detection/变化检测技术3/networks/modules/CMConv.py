...
class CMConv(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size=3, stride=1, padding=1, dilation=3, groups=1, dilation_set=4,
        ...
        def backward_hook(grad):
            out = grad.clone()
            out[self.mask] = 0  # 这里现在使用布尔掩码，应该能正常工作
            return out

        # 修改这里：使用布尔类型而不是Byte类型
        self.mask = torch.zeros(self.conv.weight.shape, dtype=torch.bool, device='cuda')
        ...
        for i in range(dilation_set):
            for j in range(groups):
                ...
                i * _in_channels: (i + 1) * _in_channels, :, :] = True  # 使用True而不是1
